...
type SqlTypeProvider(config: TypeProviderConfig) as this =
    ...
    let [<Literal>] SqlClientTypeProvider = "SqlClientTypeProvider"
    ...
    let createTypes(connectionString, conStringName,dbVendor,resolutionPath,individualsAmount,useOptionTypes,owner,caseSensitivity, tableNames, contextSchemaPath, odbcquote, sqliteLibrary, rootTypeName) =
        ...
        let rootType, prov, con =
            let rootType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly,SqlClientTypeProvider,rootTypeName,Some typeof<obj>, isErased=true)
    ...
    let paramSqlType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly, SqlClientTypeProvider, "SqlDataProvider", Some(typeof<obj>), isErased=true)
